...
import { doctors, users, partnerServices, appointments, availabilitySlots } from './shared/schema.js';
...
async function runTests() {
  ...
  try {
    ...
    .innerJoin(doctors, eq(availabilitySlots.doctorId, doctors.id))
    .innerJoin(users, eq(doctors.userId, users.id))
    .where(
      and(
        eq(availabilitySlots.isAvailable, true),
        eq(availabilitySlots.dayOfWeek, now.getDay()),
        gte(availabilitySlots.endTime, now.toTimeString().slice(0, 8))
      )
    );
    ...
    const servicesInDb = await db.select().from(partnerServices);
    ...
    if (testResults.failed > 0 || testResults.warnings.length > 0) {
      ...
      const serviceCount = await db.select({ count: count() }).from(partnerServices);
