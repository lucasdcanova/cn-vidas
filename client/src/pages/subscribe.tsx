...
export default function SubscribePage() {
  ...
  useEffect(() => {
    ...
    if (selectedPlan) {
      // Create subscription session as soon as plan is selected
      // The payment method is passed as 'card' by default, but the user can choose PIX or Boleto
      // in the Stripe PaymentElement component
      apiRequest("POST", "/api/subscription/create-session", {
        planId: Number(selectedPlan),
        paymentMethod: 'card' // Default to card, user can change in PaymentElement
      })
        .then((res) => res.json())
        .then((data) => {
          if (data.clientSecret) {
            setClientSecret(data.clientSecret);
          } else if (data.status === 'active' || data.status === 'scheduled') {
            // Free plan activated or downgrade scheduled
            toast({
            ...
            {plans && sortPlans(plans)
              ...
              .map((plan) => {
                ...
                return (
                  <Card
                    key={plan.id}
                    className={`flex flex-col h-full overflow-hidden transform transition-all duration-300 ${isUltraPlan ? "border-indigo-400 shadow-xl hover:shadow-2xl hover:-translate-y-1" :
                  ...
                  >
                    ...
                    <CardFooter className={isUltraPlan ? "bg-indigo-50" : ""}>
                      <Button
                        className={`w-full ${isUltraPlan ? "bg-gradient-to-r from-indigo-600 to-blue-800 hover:from-indigo-700 hover:to-blue-900 shadow-lg hover:shadow-indigo-200" :
